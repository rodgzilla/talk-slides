...
\begin{frame}
  ...
  Knowing how to predict the most probable word after a sequence
  requires to understand, to some extent, the \textcolor{blue}{meaning
    of words}, \textcolor{blue}{the syntax of the language} and
  \textcolor{blue}{the way concepts interact}.

  \bigskip

  Typical language models are trained on Wikipedia content or Internet
  Common Crawl.
...
\begin{frame}
  ...
  Some researchers discovered that by asking the model to predict the
...
\begin{frame}
  ...
  To apply this algorithm, we do not need the a label for the clean
  image, we just assume that the model is already giving the right one
  and improve it by forcing it to acquire \textcolor{red}{noise
    invariance}.
